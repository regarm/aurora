...
function run(ws, data){
	createSpace(function(err, space){
		...
		var interpreter = '';
		var sourceTarget = path.join(space, 'source.' + Cache.LangsCache.get(data.lang).source_ext);
		var exeTarget = '';
		if(Cache.LangsCache.get(data.lang).compile_template){
			if(data.lang === 'JAVA'){
				exeTarget = 'Main';
			} else {
		...
		fs.writeFile(inputTarget, data.input, function (err){
			...
			fs.writeFile(sourceTarget, data.source, function (err){
				...
				compile(data.lang, space, sourceTarget, exeTarget, function (compile_log){
					if(compile_log.VERDICT){
						send(ws, 'conclude', 'success');
						send(ws, 'result', compile_log);
					} else {
						send(ws, 'status', 'running');
						execute(data.lang, space, exeTarget, inputTarget, outputTarget, errorTarget, 3, 131072, 16384, function (execute_log){
